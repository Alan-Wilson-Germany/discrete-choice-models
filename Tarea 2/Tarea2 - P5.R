...
# Asumiendo predictions_base ya cargado con las probabilidades

# Top 10 y worst 10 para DEL
TOP10_DEL <- predictions_base %>% arrange(desc(DEL)) %>% slice_head(n = 10)
PEORES10_DEL <- predictions_base %>% arrange(DEL) %>% slice_head(n = 10)

# Top 10 y worst 10 para VCP
TOP10_VCP <- predictions_base %>% arrange(desc(VCP)) %>% slice_head(n = 10)
PEORES10_VCP <- predictions_base %>% arrange(VCP) %>% slice_head(n = 10)

# Top 10 y worst 10 para HM
TOP10_HM <- predictions_base %>% arrange(desc(HM)) %>% slice_head(n = 10)
PEORES10_HM <- predictions_base %>% arrange(HM) %>% slice_head(n = 10)

# Probabilidad promedio para cada alternativa
mean_DEL <- mean(predictions_base$DEL)
mean_VCP <- mean(predictions_base$VCP)
mean_HM <- mean(predictions_base$HM)

# Mostrar resultados
list(
  TOP10_DEL = TOP10_DEL,
  PEORES10_DEL = PEORES10_DEL,
  TOP10_VCP = TOP10_VCP,
  PEORES10_VCP = PEORES10_VCP,
  TOP10_HM = TOP10_HM,
  PEORES10_HM = PEORES10_HM,
  promedio_prob = c(DEL = mean_DEL, VCP = mean_VCP, HM = mean_HM)
)
### Print outputs of additional diagnostics to new output file (remember to close file writing when complete)
apollo_sink()

sink("OUTPUT T2-P5.txt")
apollo_modelOutput(model)           
sink()       